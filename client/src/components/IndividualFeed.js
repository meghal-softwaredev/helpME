...
function IndividualFeed() {
  ...
  const individualFeedDetails = useSelector(
    (state) => state.individualFeedDetails
  );
  ...
  const handleSubmit = (event) => {
    ...
    dispatch(saveAnswer({ feedId, newAnswer }));
    setNewAnswer("");
    setEditorState("");
    dispatch(getFeedAnswers(feedId));
  };
      ...
      <div className="feed-item-container">
        {loadingFeedDetails ? (
          ...
          feed && (
            ...
            >
              <Box
                sx={{ fontSize: "h6.fontSize", fontWeight: "medium", mb: 2 }}
              >
                {feed.title}
              </Box>
              <Typography className="feed-description">
                {convert(feed.description)}
              </Typography>
              <Box>
                {feed.tags.map((tag) => (
                  <Chip
              ...
              <Box sx={{ display: "flex", alignItems: "center" }}>
                <Box sx={{ flex: "1" }}>
                  <Typography>
                    Posted by <b>{feed.user && feed.user.name}</b> on{" "}
                    {moment(feed.createdAt).format("MMMM Do, YYYY")}
                  </Typography>
                </Box>
                <Box
                  sx={{
              ...
              {userInfo && feed.user && feed.user._id === userInfo._id && (
                <CardActions>
                  <IconButton
                    ...
                    component="span"
                    onClick={() => handleOpenDialog({ msg: "EditFeed" })}
                  >
                    <EditIcon />
                  </IconButton>
                  <IconButton
                    ...
                    component="span"
                    onClick={() => handleOpenDialog({ msg: "DeleteFeed" })}
                  >
                    <DeleteIcon />
                  </IconButton>
                </CardActions>
